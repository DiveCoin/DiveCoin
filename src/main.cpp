...
#if defined(NDEBUG)
# error "DiveCoin cannot be compiled without assertions."
#endif
...
static const int64_t nStartSubsidy = 60 * COIN;
...
int64_t GetBlockValue(int nBits, int nHeight, int64_t nFees)
{
    ...
    if(nHeight == 1) {nSubsidy = 360000 * COIN;}
    else if (nHeight <= 1000) {nSubsidy = 1 * COIN;}
    else if (nSubsidy < nMinSubsidy){nSubsidy = nMinSubsidy;}
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue)
{
    ...
    if (nHeight >= 100000){ret = blockValue * 50 / 100;} 
    else if (nHeight >= 25000){ ret = blockValue * 25 / 100;} 
    else{ret = blockValue * 10 / 100;}
...
static const int64_t nInterval = 1; // DiveCoin: retarget every block
...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    // current difficulty formula, divecoin - DarkGravity v3, written by Evan Duffield - evan@divecoinpay.io 
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
...
unsigned int GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
        ...
        if (retarget == DIFF_BTC)
        {
            ...
            // DiveCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
void ThreadScriptCheck() {
    RenameThread("divecoin-scriptch");
