...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("DiveCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("DiveCoinMiner : ProcessBlock, block not accepted");
    }
...
{
    LogPrintf("DiveCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("divecoin-miner");
    ...
    try { while (true) {
        ...
        LogPrintf("Running DiveCoinMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("DiveCoinMiner terminated\n");
        throw;
    }
}
