...
//int64_t enforceMasternodePaymentsTime = 4085657524; // DiveCoin
...
void ParseParameters(int argc, const char* const argv[])
{
    ...
    BOOST_FOREACH(const PAIRTYPE(string,string)& entry, mapArgs)
    {
        ...
        if (name.find("--") == 0)
        {
            std::string singleDiveCoin(name.begin()+1, name.end());
            if (mapArgs.count(singleDiveCoin) == 0)
                mapArgs[singleDiveCoin] = entry.second;
            name = singleDiveCoin;
        }
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\DiveCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\DiveCoin
    // Mac: ~/Library/Application Support/DiveCoin
    // Unix: ~/.divecoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "DiveCoin";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "divecoin.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty divecoin.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override divecoin.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "divecoind.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong DiveCoin will not work properly.");
